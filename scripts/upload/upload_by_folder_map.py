...
根据本地文件目录结构，自动匹配远程folder映射，上传到对应的知识库。

与 upload_to_folder.py 的区别：
- upload_to_folder.py: 直接指定远程目录ID，所有文件上传到同一个目录
- upload_by_folder_map.py: 根据本地目录结构自动匹配远程目录映射
"""
...
# 配置日志文件名（按日期）
log_filename = os.path.join(logs_dir, f"upload_by_folder_map_{datetime.now().strftime('%Y-%m-%d')}.log")
...
log = logging.getLogger("upload_by_folder_map")
...
def retry_on_connection_error(max_retries=3, base_delay=2):
    ...
    def decorator(func):
        def wrapper(*args, **kwargs):
            ...
            thread_log = logging.getLogger(f"upload_by_folder_map-{thread_name}")
...
if stats['error_count'] > 0:
    ...
    log.info(f"可运行 retry_failed.py 重新上传失败的文件")
